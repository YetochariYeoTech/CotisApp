    ...
    <div className="p-4">
      ...
      <div className="overflow-x-auto bg-base-100 rounded-box shadow-xl">
        <table className="table w-full">
          <thead>
            <tr>
              <th className="font-sans">ID Membre</th>
              <th className="font-sans">Montant</th>
              <th className="font-sans">Type</th>
              <th className="font-sans">Statut</th>
              <th className="font-sans">Date</th>
              {user?.role === Role.TREASURER && <th className="font-sans">Actions</th>}
            </tr>
          </thead>
          <tbody>
            {/* Placeholder for transactions list */}
            <tr>
              <td className="font-sans">member123</td>
              <td className="font-sans">100</td>
              <td className="font-sans">DUES</td>
              <td className="font-sans">PENDING</td>
              <td className="font-sans">2025-10-08</td>
              {user?.role === Role.TREASURER && (
                <td>
                  <button className="btn btn-sm btn-success font-sans" onClick={() => handleValidateTransaction('transaction123')}>
                    Valider
                  </button>
                </td>
              )}
